...
def splitArray(nums):
  b = []
  return splitArrayHelper (nums, b, 0)
...
def splitArrayHelper(a, b, idx):
  if (idx == len(a)):
    ...
    c = b[:]
    b.append (a[idx])
    return splitArrayHelper(a, b, idx+1) or splitArrayHelper(a, c, idx+1)
...
def splitOdd10Helper(nums, i, group1, group2):
    if (i == len(nums)):
        return (group1 % 2 ==1 and group2 %10 == 0 or group2 % 2 == 1 and group1 % 10 == 0)
    if (splitOdd10Helper(nums, i + 1, group1 + nums[i], group2)):
        return True
    return splitOdd10Helper(nums, i + 1, group1, group2 + nums[i])
...
def split53Helper(nums, i, x):
  if (i == len(nums)):
    return (x == 0)
  if (nums[i] % 5 == 0):
    return split53Helper(nums, i+1, x + nums[i])
  if (nums[i] % 3 == 0):
    return split53Helper(nums, i+1, x - nums[i])
  if (split53Helper(nums, i+1, x + nums[i])):
    return True
  else:
    return split53Helper(nums, i+1, x - nums[i])
